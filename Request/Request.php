...
class Request
{
     ...
     * @throws Exception
     */
    public function deleteRequest($url, $parameters = [])
    {
        if (!empty($parameters)) {
            $this->parameters->addDelete($parameters);
        }
        return $this->request($url);
    }
    ...
    public function request($url, $modified = null)
    {
        ...
        } else if($this->parameters->hasDelete()) {
            $fields = json_encode($this->parameters->getDelete());
